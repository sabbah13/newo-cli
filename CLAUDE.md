...
- `commands/` - Individual command handlers (45 modular commands):
  ...
  - **Project version (v3.7.7):** `update-project.ts` - set displayed project `version` + `--force-update` from registry
...
- `PATCH /api/v1/designer/projects/by-id/{projectId}` - Update project meta incl. `version` (full-object PATCH; NEW v3.7.7, replaces the dead `PUT /projects/{id}`)
- `POST /api/v1/designer/projects/by-id/{projectId}/force-update` - Force-update project content from its registry (NEW v3.7.7)
