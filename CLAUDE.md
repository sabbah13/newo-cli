...
- ✅ **Sandbox & point-edit toolkit** (NEW v3.7.5) - Addresses four gaps found during PR #2076 superagent testing. R1: `newo sandbox --connector <idn>` / `--integration <idn>` / `--list-connectors` select which running connector to chat through (previously hardcoded to the first one, making e.g. `vibe_agent` unreachable). R2: `--file` / `--stdin` for 40k–400k-char messages, `--timeout <sec>` (default stays 60), `--json` with user+agent `external_event_id` for `newo logs --event-id` correlation. R3: `newo get-skill` / `newo update-skill` — point inspection/edit of one skill (model, script) by IDN path without a pulled workspace, optional `--publish`. R4: `newo logs --name <ActionName>` client-side filter on `data.name`; help documents that turn model lives in `data.source.model`. New module `src/sync/remote-skill.ts`; 20 unit tests across `test/sandbox-skill-commands.test.js` (14) and `test/logs-command.test.js` (6)
- ✅ **Conversation by session** (NEW v3.7.6) - `newo conversations --session-id <uuid>` pulls the act chronicle (dialog transcript) for one conversation session by its platform `session_id`. Two-step resolution that works with an api-key token: `listUserPersonas(sessionId)` → `user-personas?session_id=…` resolves the persona/actor, then `getChatHistory(user_actor_id)` → `chat/history` returns the transcript (the direct `acts?session_id=…` endpoint needs a logged-in user token). Transcript scoped to the resolved actor(s); service actors (`program_timer`/`magic_browser`) excluded. Default writes `conversation-{session_id}.yaml`; `--json` emits `{session_id, personas, actor_ids, total_acts, acts[]}`. New `pullConversationBySession()` in `src/sync/conversations.ts`; 4 unit tests in `test/conversations-session.test.js`. Short alias **`newo session <uuid>`** (`src/cli/commands/session.ts`) wraps this — **dialog is the default and the recommended view** (fast, one `chat/history` fetch; for some sessions the resolved actor's chat/history also includes the agent `THOUGHTS` and system-log lines, but this varies by session/persona — the richer narrative is in the UI-only `acts` layer). **`--full`** (`pullSessionFull`) additionally merges the `analytics/logs` execution trace (skill/NSL calls, `flow_idn`/`skill_idn`/model/timing) into one chronological `timeline`, scoped to the actor within the dialog time window (chat items carry only `external_event_id`, no `session_id`); bounded by `--max-logs` (default 20000, since a busy session has tens of thousands of calls) with progress output and a `partial` flag. The full Builder-UI chronicle (`acts?session_id=…`) is NOT api-key reachable (empty `account_id` → hangs); see `docs/SESSION_CHRONICLE_PLATFORM_ASK.md`. 3 unit tests in `test/conversations-full.test.js`
- ✅ **Project version & force-update** (NEW v3.7.6) - `newo update-project <project-idn>` sets a customer project's displayed Builder version after a deploy so the account label matches the deployed template. The displayed value is the project's `version` field — NOT `registry_item_version`, and NOT content push (`newo push` never writes it). The single-project resource is `by-id/{id}` and its PATCH is not a true partial (an empty body flips `is_auto_update_enabled` to `false`), so the command GETs the current meta, overlays only the requested fields, and PATCHes the full object back. `--force-update` fires the Builder's "Force Update Project" action (`POST /api/v1/designer/projects/by-id/{id}/force-update`, empty body → 200 `null`), re-syncing content from the registry; runs before the version PATCH so `--version` still wins the label. `--version` is optional when `--force-update` is passed. Fixes the stale `updateProject` route in `src/api.ts` (`PUT /projects/{id}` → 404, now `PATCH /projects/by-id/{id}`) and adds `version`+`idn` to its allowed fields; adds `forceUpdateProject()`. New `src/cli/commands/update-project.ts` with pure helpers `buildProjectUpdateBody`/`applyProjectUpdate`; 8 unit tests in `test/update-project.test.js`
- ✅ **JSON-attr Markdown escape fidelity** (NEW v3.7.7) - `fixInvalidJsonEscapes` now *escapes* invalid JSON escapes (`\_` → `\\_`) instead of *dropping* the backslash (`\_` → `_`). Both produce valid JSON (the v3.7.3 blank-Builder fix holds), but only escaping is byte-faithful: dropping turned Markdown-escaped underscores into emphasis/italics in the Workflow Builder (observed on a live customer canvas). `JSON.parse` decodes `\\_` back to the literal `\_`. A pull/push cycle now restores the previously-dropped backslash, so the first push after upgrading may rewrite affected canvas attributes once; change-detection (`jsonValuesEqual`) still canonicalizes both sides so steady-state pushes stay idempotent. Reported by Bob in PR [#11](https://github.com/sabbah13/newo-cli/pull/11); 25 regression tests in `test/json-attribute-roundtrip.test.js`. See `src/sync/json-attr-utils.ts`
- ✅ **Attribute point-edit, metadata-faithful** (NEW, Unreleased) - `newo update-attribute <idn>` upgraded to round-trip the FULL attribute object on write (GET current → overlay only passed flags → PUT whole object), instead of the fixed 8-field subset the original command sent. The old path went through `updateCustomerAttribute`, which omitted `is_read_only` (and any field the platform added), silently resetting it on every value edit — the Builder UI's own request (reference curl) sends the complete object. Adds `--project <idn>` for PROJECT attributes (`PUT /api/v1/designer/projects/{projectId}/attributes/{id}`) alongside customer attributes, `--file <path>` for large/multi-line values, and `--json` output. JSON-typed values (`value_type: json`) are normalized to a canvas-safe compact string via `normalizeJsonValueForStorage`, and `--value 0 / "" / False` are honored (undefined-check, not falsy-check). New API helpers `putCustomerAttributeRaw` / `putProjectAttributeRaw` in `src/api.ts`; new pure helper `buildAttributeUpdateBody` in `src/cli/commands/update-attribute.ts`; 10 unit tests in `test/update-attribute.test.js`
...
- `commands/` - Individual command handlers (45 modular commands):
  ...
  - **Project version (v3.7.6):** `update-project.ts` - set displayed project `version` + `--force-update` from registry
...
- `conversations.ts` - Conversation history management (v3.7.6: `pullConversationBySession` for single-session transcript via `--session-id` / `newo session <uuid>`; `pullSessionFull` for `--full` = dialog merged with the `analytics/logs` execution trace, bounded by `--max-logs`)
...
npx newo update-project <idn> --version <semver>               # Set displayed Builder project version (post-deploy)
npx newo update-project <idn> --force-update                   # Force-update project content from its registry
npx newo update-project <idn> --force-update --version <v> --json  # Force-update then label, machine-readable result
...
- `PATCH /api/v1/designer/projects/by-id/{projectId}` - Update project meta incl. `version` (full-object PATCH; NEW v3.7.6, replaces the dead `PUT /projects/{id}`)
- `POST /api/v1/designer/projects/by-id/{projectId}/force-update` - Force-update project content from its registry (NEW v3.7.6)
