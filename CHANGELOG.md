...
- **`session_id` support on `listUserPersonas()`** — now accepts an optional `sessionId` forwarded as the `session_id` query param to `GET /api/v1/bff/conversations/user-personas`, which server-side filters to the persona(s) tied to that session (verified: 1 persona with the filter vs the full page without it).
- **`newo session <uuid>`** — short command to inspect one session by its platform `session_id`. **Dialog is the default and the recommended view**: one fast `chat/history` fetch. For some sessions the resolved actor's history also includes the agent's `THOUGHTS` reasoning and system-log lines (booking payloads, availability, SMS, end-of-session summary); for others it returns only the spoken turns (the richer narrative is in the UI-only `acts` layer, not api-key-reachable). Writes `conversation-{session_id}.yaml`; `--json` emits `{session_id, personas, actor_ids, total_acts, acts[]}`. Thin wrapper in `src/cli/commands/session.ts` over `conversations --session-id` (identical behavior; `newo session <uuid>` == `conversations --session-id <uuid>`).
- **`newo session <uuid> --full`** — opt-in heavier view: merges the dialog with the execution trace (`analytics/logs` scoped to the session's actor(s) within the dialog time window) into one chronological `timeline[]` of `kind: message` and `kind: call`/`operation` entries (every NSL/skill call + LLM `Gen`, with `flow_idn`, `skill_idn`, `model`, timing), linked by `external_event_id`. Writes `conversation-{session_id}-full.yaml` (console prints the dialog inline + a trace summary). `--max-logs <n>` bounds the trace fetch (default 20000 — a busy session can have tens of thousands of skill calls; sets `partial: true` when hit); `--pad-end <min>` controls how far past the last turn logs are collected (default 10). Progress is printed as personas/messages/logs are gathered (suppressed under `--json`). The full Builder UI chronicle (`bff/conversations/acts` — formatted `analyze_conversation`, recordings) is **not** api-key-reachable (its token's `account_id` claim is empty → the endpoint hangs); see `docs/SESSION_CHRONICLE_PLATFORM_ASK.md`. New `pullSessionFull()` in `src/sync/conversations.ts`; 3 unit tests in `test/conversations-full.test.js`.
